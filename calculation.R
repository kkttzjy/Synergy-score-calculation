...
analyze = function(raw.data.files = "20181019 resazurin R.xlsx", 
                   info.data.files = "20181019 resazurin infomation.xlsx",
                   raw.data.files.name = "20181019 resazurin R"){
  #read raw data
  data = list()
  ...
  for ( k in 1:n){
    ...
    # cutoff to filter positive control
    poscontrol = poscontrol[which(poscontrol>25000)]
    ...
    data_plot = as.data.frame(cbind(concentration, concentration_log, 
                                    ...
                                    C4_1, C4_2, C4_3, C4_4))
    agent2_mean = rowMeans(data_plot[,3:6])
    agent1_mean = rowMeans(data_plot[,7:10])
    C1_mean = rowMeans(data_plot[,11:14])
    C2_mean = rowMeans(data_plot[,15:18])
    C3_mean = rowMeans(data_plot[,18:22])
    C4_mean = rowMeans(data_plot[,23:26])
...
Score = function(raw.data.files.name = "20181019 resazurin R",
                 info.data.files = "20181019 resazurin infomation.xlsx") {
  information = read.xlsx(xlsxFile = info.data.files, colNames = T) 
  agent1 = information$Agent1
  agent1 = agent1[!is.na(agent1)]
  ...
  for ( k in 1:m){
    ...
    PE = E
    PE[PE<0] = 0
    V[k] = sum(apply(E,1,sum))/100 ## inhibition volume
    S[k] = sum(apply(PE*D,1,sum))*log(2)*log(2)/10000 ## Chalice score
  }
  ...
  for ( k in 1:m){
    ...
    maxlimy = round(max(data_plot_all$res), digits = 1)
